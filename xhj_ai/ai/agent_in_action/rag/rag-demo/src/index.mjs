...
// 提供内存向量存储  rag 学习，或轻量级的向量数据
// psql
// 帮我安装PostgreSQL root 密码设置为123456 开启向量存储拓展
...
async function main() {
  ...
  // 存入到内存中
  // 将上述documents 数组中的所有文本 通过embedding 转换成向量，并存入内存，可以拥有一个语义搜索的知识库
  const vectorStore = await MemoryVectorStore
    .fromDocuments(documents, embedding);

  console.log(vectorStore);

  // 将向量数据库转换为检索器 —— 语义检索的标准入口，输入问题，输出最相关的文档列表
  ...
  // 检索：内部将 question 转为向量，在向量数据库中计算距离，返回 k 个 Document 对象
  ...
  docs.forEach((doc,i) => {
    ...
    // retriever 过滤，rerank
    // 1-score 值越大越相似，使其与 cosine 相似度一致
